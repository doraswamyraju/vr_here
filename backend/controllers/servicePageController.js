...
const getServicePageById = asyncHandler(async (req, res) => {
    ...
    if (!page) {
        if (DEFAULT_CONFIGS[pageId] || DEFAULT_CONFIGS[baseSlug]) {
            const seedConfig = DEFAULT_CONFIGS[pageId] || DEFAULT_CONFIGS[baseSlug];
            page = await ServicePageConfig.create(seedConfig);
        } else {
            page = await ServicePageConfig.create({
                pageId: baseSlug || pageId,
                title: (baseSlug || pageId).split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' '),
                description: `Dynamic compliance and setup services for ${baseSlug || pageId}.`,
                packages: DEFAULT_CONFIGS['private-limited'].packages,
                faqs: DEFAULT_CONFIGS['private-limited'].faqs,
                reviews: DEFAULT_CONFIGS['private-limited'].reviews,
                steps: DEFAULT_CONFIGS['private-limited'].steps
            });
        }
    }
