...
import { llpConfig } from '../backend/data/serviceConfigs/llp.js';

const PAGE_ID = 'llp-registration';

const RELATED_SERVICES = [
  {
    title: "Private Limited Company",
    price: "₹6,499",
    desc: "Launch your startup with equity share capital and maximum venture investor appeal.",
    link: "/pvt-ltd-registration"
  },
  {
    title: "Partnership Firm",
    price: "₹3,499",
    desc: "Quick deed drafting, notary execution, and Registrar of Firms (ROF) certification.",
    link: "/partnership-firm-registration"
  },
  {
    title: "One Person Company (OPC)",
    price: "₹5,999",
    desc: "Solo founder corporate entity with 100% equity ownership and limited liability protection.",
    link: "/one-person-company"
  },
  {
    title: "GST Registration & Filing",
    price: "₹2,569",
    desc: "Fast GSTIN issuance and return filings handled by experienced CA specialists.",
...
const LLPRegistrationPage = () => {
  ...
  const [pageConfig, setPageConfig] = useState(llpConfig);
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      const mainEl = document.getElementById('llp-container');
      if (mainEl) {
  ...
  const activeHero = pageConfig?.hero || llpConfig.hero;
  const activeStats = pageConfig?.stats || llpConfig.stats;
  const activeLogos = pageConfig?.logos || llpConfig.logos;
  const activePackages = pageConfig?.packages || llpConfig.packages;
  const activeReviews = pageConfig?.reviews || llpConfig.reviews;
  const activeSteps = pageConfig?.steps || llpConfig.steps;
  const activeFaqs = pageConfig?.faqs || llpConfig.faqs;
  const activeGuide = pageConfig?.guide || llpConfig.guide;
  const activePopularSearches = pageConfig?.popularSearches || llpConfig.popularSearches;
    ...
    launchRazorpayCheckout({
      ...
      onSuccess: async (data) => {
        ...
        await showPaymentSuccessPopup({
          serviceName: selectedPlan?.name || data?.order?.serviceName || 'LLP Registration',
  ...
  return (
    <div className={`font-sans text-slate-800 bg-white min-h-screen selection:bg-red-100 selection:text-red-900 overflow-x-hidden ${isAuthorized ? 'pt-14' : ''}`}>
      ...
      <ConsultationPaymentModal
        ...
        title={selectedPlan?.id === 'consultation' ? 'Book LLP Consultation' : 'Select LLP Package'}
        initialTermsAccepted={false}
      />

      <div id="llp-container" className="animate-fade-in">
        {/* 1. Hero Section */}
          ...
          <div className="max-w-7xl mx-auto px-4 relative z-10 flex flex-col lg:flex-row items-center gap-12">
            <div className="lg:w-1/2">
              <div className="inline-flex items-center px-4 py-1.5 rounded-full bg-white border border-slate-200 shadow-sm text-sm font-bold text-slate-600 mb-6">
                ...
                {sanitizeText(activeHero.badgeText, pageConfig?.city) || 'Zero Audit Under ₹40 Lakh Turnover'}
              </div>
              <h1 className="text-4xl lg:text-6xl font-black text-slate-900 tracking-tight leading-[1.1] mb-6">
                {sanitizeText(activeHero.title, pageConfig?.city) || 'Register Your LLP Online'}
              </h1>
              <p className="text-xl text-slate-600 leading-relaxed mb-8">
                {sanitizeText(activeHero.subtitle, pageConfig?.city) || 'The perfect blend of a partnership and private limited company for professionals and partners.'}
              </p>
              ...
              <div className="flex flex-col sm:flex-row gap-4 mb-8">
                ...
                >
                  Book CA/CS Call @ ₹{activeHero.consultationPrice || 499} <ArrowRight className="ml-2 w-5 h-5" />
                </button>
              ...
              <div className="flex items-center space-x-2 text-sm font-medium text-slate-500">
                ...
                <span>Consultation fee of ₹{activeHero.consultationPrice || 499} is 100% adjustable in final package</span>
              </div>
            </div>
            ...
            <div className="lg:w-1/2 relative hidden lg:block">
              <div className="relative z-10 bg-white p-8 rounded-2xl shadow-2xl border border-slate-100 transform rotate-2 hover:rotate-0 transition duration-500">
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <h3 className="text-xl font-bold text-slate-900">LLP Formation Suite</h3>
                    <p className="text-slate-500 text-sm">All-inclusive setup starting at ₹3,999</p>
                  </div>
                  <div className="bg-green-100 text-green-700 font-bold px-3 py-1 rounded text-xs">LLP ACT 2008</div>
                </div>
                <div className="space-y-4">
                  {['2 DPINs & 2 Digital Signatures (DSC)', 'RUN-LLP Name Reservation', 'FiLLiP Incorporation Filing', 'Customized LLP Agreement (Form 3)', 'LLP PAN, TAN & MSME Registration'].map((item, i) => (
        ...
        <div className="bg-slate-900 py-12 relative overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto px-4 mb-6 text-center">
            <p className="text-xs uppercase font-black tracking-widest text-red-400">
              Trusted By Over 5,000+ Fast-Growing Indian Startups & LLPs
            </p>
          </div>
        ...
        <section className="py-20 bg-white border-y border-slate-100">
          <div className="max-w-7xl mx-auto px-4">
            <div className="text-center mb-16">
              ...
              <h2 className="text-3xl lg:text-5xl font-black text-slate-900 mt-4 tracking-tight">What Our Partners Say</h2>
              <p className="text-lg text-slate-600 mt-2 font-medium">Hear directly from LLPs registered and powered by VR Here.</p>
            </div>
        ...
        <section className="py-24 bg-slate-50">
          <div className="max-w-6xl mx-auto px-4">
            <div className="text-center mb-20">
              ...
              <p className="text-lg text-slate-600 mt-2 font-medium">Get your LLP registered and Form 3 agreement filed 100% online.</p>
            </div>
          ...
          <div className="max-w-7xl mx-auto px-4 flex flex-col md:flex-row gap-12 items-center">
            <div className="md:w-1/2">
              ...
              <p className="text-slate-600 mb-8 leading-relaxed font-medium">
                LLP is the most popular vehicle for consultants, partners, and service professionals who need limited liability protection with lower annual costs.
              </p>
              <div className="grid grid-cols-1 gap-4">
                {[
                  { t: "Limited Partner Liability", d: "Partners are not personally liable for debts or negligence of other partners." },
                  { t: "Audit Exemption Benefit", d: "No mandatory audit until turnover crosses ₹40 Lakhs or contribution crosses ₹25 Lakhs." },
                  { t: "Zero Dividend Tax", d: "Profits can be directly distributed to partners without dividend distribution taxes." },
                  { t: "Low Maintenance Cost", d: "Fewer statutory registers and simpler ROC filings compared to private companies." }
                ].map((item, i) => (
            ...
            <div className="md:w-1/2">
              <div className="bg-slate-50 p-8 rounded-3xl border border-slate-200 shadow-sm">
                <h3 className="text-xl font-black text-slate-900 mb-6 text-center">LLP Minimum Requirements</h3>
                <ul className="space-y-4">
                  {[
                    "Minimum 2 Designated Partners",
                    "At least 1 Indian Resident Designated Partner",
                    "Digital Signature Certificates (Class 3 DSC) for partners",
                    "Execution & filing of Form 3 LLP Agreement within 30 days",
              ...
              <div className="mt-12 text-left bg-slate-50 p-8 md:p-12 rounded-3xl border border-slate-200 animate-in fade-in slide-in-from-top-4 duration-300 grid grid-cols-1 lg:grid-cols-3 gap-12">
                ...
                <div className="lg:col-span-1 space-y-6 max-h-[500px] overflow-y-auto pr-4 scrollbar-thin scrollbar-thumb-slate-300">
                  <h3 className="text-xl font-black text-slate-900 border-b-2 border-red-500 pb-2">
                    {activeGuide.title || 'Guide to LLP Registration'}
                  </h3>
