          ...
          return (
            <div key={proj._id} onClick={() => { setSelectedProject(proj); setCurrentView('detail'); }} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md cursor-pointer group flex flex-col justify-between">
              <div>
                ...
                <p className="text-xs text-slate-400 mb-4">Value: ₹{proj.price.toLocaleString()}</p>
              </div>
        ...
        <div className="bg-white p-6 border-b border-slate-200 flex justify-between items-center sticky top-0 z-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-slate-500 text-sm flex items-center mt-1"><Building2 size={14} className="mr-1" /> {project.user?.name} <span className="mx-2 text-slate-300">|</span> <span className="text-indigo-600 font-medium">Value: ₹ {project.price.toLocaleString()}</span></p>
            </div>
          </div>
        </div>
