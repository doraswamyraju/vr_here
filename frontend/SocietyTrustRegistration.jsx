...
import { societyTrustConfig } from '../backend/data/serviceConfigs/societyTrust.js';

const PAGE_ID = 'society-trust-registration';

const RELATED_SERVICES = [
  {
    title: "Section 8 Company (NGO)",
    price: "₹12,499",
    desc: "National non-profit corporate status with CSR funding eligibility and MCA governance.",
    link: "/section-8-company"
  },
  {
    title: "12A & 80G Tax Exemption",
    price: "₹9,999",
    desc: "Income tax exemption for your NGO and 50% donor tax relief receipts under IT Act.",
    link: "/contact?service=12A+80G"
  },
  {
    title: "Private Limited Company",
    price: "₹6,499",
    desc: "Start a commercial enterprise to fund and scale social tech initiatives.",
    link: "/pvt-ltd-registration"
  },
  {
    title: "NITI Aayog NGO DARPAN",
    price: "₹2,499",
    desc: "Mandatory unique ID to receive central and state government grants.",
    link: "/contact?service=DARPAN"
  }
];
...
const SocietyTrustRegistrationPage = () => {
  const navigate = useNavigate();

  const userInfo = JSON.parse(localStorage.getItem('userInfo') || 'null');
  const isAuthorized = userInfo && (userInfo.role === 'admin' || userInfo.role === 'employee');

  const [pageConfig, setPageConfig] = useState(societyTrustConfig);
  ...
  const activeHero = pageConfig?.hero || societyTrustConfig.hero;
  const activeStats = pageConfig?.stats || societyTrustConfig.stats;
  const activeLogos = pageConfig?.logos || societyTrustConfig.logos;
  const activePackages = pageConfig?.packages || societyTrustConfig.packages;
  const activeReviews = pageConfig?.reviews || societyTrustConfig.reviews;
  const activeSteps = pageConfig?.steps || societyTrustConfig.steps;
  const activeFaqs = pageConfig?.faqs || societyTrustConfig.faqs;
  const activeGuide = pageConfig?.guide || societyTrustConfig.guide;
  const activePopularSearches = pageConfig?.popularSearches || societyTrustConfig.popularSearches;
  ...
  const handleFormSubmit = ({ formData: submittedFormData, termsAccepted }) => {
    ...
    launchRazorpayCheckout({
      serviceName: 'Society & Trust Registration',
      selectedPlan,
      ...
      onSuccess: async (data) => {
        ...
        await showPaymentSuccessPopup({
          serviceName: selectedPlan?.name || data?.order?.serviceName || 'Society & Trust Registration',
  ...
  return (
    <div className="font-sans text-slate-800 bg-white min-h-screen selection:bg-red-100 selection:text-red-900 overflow-x-hidden">
      ...
      <ConsultationPaymentModal
        ...
        title={selectedPlan?.id === 'consultation' ? 'Book Trust / Society Advisory' : 'Select NGO Package'}
        initialTermsAccepted={false}
      />

      <div id="society-trust-container" className="animate-fade-in">
        {/* 1. Hero Section */}
          ...
          <div className="max-w-7xl mx-auto px-4 relative z-10 flex flex-col lg:flex-row items-center gap-12">
            <div className="lg:w-1/2">
              <div className="inline-flex items-center px-4 py-1.5 rounded-full bg-white border border-slate-200 shadow-sm text-sm font-bold text-slate-600 mb-6">
                ...
                {sanitizeText(activeHero.badgeText, pageConfig?.city) || 'Indian Trusts Act 1882 / Societies Act'}
              </div>
              <h1 className="text-4xl lg:text-6xl font-black text-slate-900 tracking-tight leading-[1.1] mb-6">
                {sanitizeText(activeHero.title, pageConfig?.city) || 'Register Society or Charitable Trust Online'}
              </h1>
              <p className="text-xl text-slate-600 leading-relaxed mb-8">
                {sanitizeText(activeHero.subtitle, pageConfig?.city) || 'Establish your charitable, educational, or religious organization with legal trust deed drafting and government registration.'}
              </p>
              ...
              <div className="flex flex-col sm:flex-row gap-4 mb-8">
                ...
                >
                  Book Advisory Call @ ₹{activeHero.consultationPrice || 999} <ArrowRight className="ml-2 w-5 h-5" />
                </button>
            ...
            <div className="lg:w-1/2 relative hidden lg:block">
              <div className="relative z-10 bg-white p-8 rounded-2xl shadow-2xl border border-slate-100 transform rotate-2 hover:rotate-0 transition duration-500">
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <h3 className="text-xl font-bold text-slate-900">Trust & Society Setup</h3>
                    <p className="text-slate-500 text-sm">All-inclusive registration from ₹8,499</p>
                  </div>
                  <div className="bg-green-100 text-green-700 font-bold px-3 py-1 rounded text-xs">SUB-REGISTRAR</div>
                </div>
                <div className="space-y-4">
                  {['Custom Public Charitable Trust Deed Drafting', 'Society MoA & By-laws Preparation', 'Sub-Registrar Execution Support', 'Entity PAN, TAN & Bank Resolution Kit', 'Provisional 12A & 80G Tax Exemption Filing'].map((item, i) => (
        ...
        <div className="bg-slate-900 py-12 relative overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto px-4 mb-6 text-center">
            <p className="text-xs uppercase font-black tracking-widest text-red-400">
              Trusted By Over 1,000+ Charitable Trusts, Societies & Educational Institutions
            </p>
          </div>
                ...
                >
                  ...
                  <div className="text-4xl font-black text-slate-900 mb-6">
                    ...
                    <span className="text-[10px] font-bold text-slate-500 ml-1 block mt-1">
                      {pkg.isAdjustable ? '(Fully Adjustable)' : '+ Sub-Registrar / ROF Stamp Duty'}
                    </span>
                  </div>
        ...
        <section className="py-20 bg-white border-y border-slate-100">
          <div className="max-w-7xl mx-auto px-4">
            <div className="text-center mb-16">
              ...
              <h2 className="text-3xl lg:text-5xl font-black text-slate-900 mt-4 tracking-tight">What Trustees Say</h2>
              <p className="text-lg text-slate-600 mt-2 font-medium">Hear directly from trust and society management committees established through VR Here.</p>
            </div>
        ...
        <section className="py-24 bg-slate-50">
          <div className="max-w-6xl mx-auto px-4">
            <div className="text-center mb-20">
              <span className="text-xs uppercase font-black tracking-widest text-red-600 bg-red-50 px-3 py-1.5 rounded-full font-bold">Registration Flow</span>
              <h2 className="text-3xl lg:text-5xl font-black text-slate-900 mt-4 tracking-tight">Redesigned, Effortless Steps</h2>
              <p className="text-lg text-slate-600 mt-2 font-medium">Deed drafting, sub-registrar execution, and 12A/80G filing managed smoothly.</p>
            </div>
        ...
        {/* 6. Why Trust/Society & Requirements */}
          ...
          <div className="max-w-7xl mx-auto px-4 flex flex-col md:flex-row gap-12 items-center">
            <div className="md:w-1/2">
              <h2 className="text-3xl lg:text-5xl font-black text-slate-900 mb-6 tracking-tight">
                Why Form a Trust or Society?
              </h2>
              <p className="text-slate-600 mb-8 leading-relaxed font-medium">
                Charitable Trusts and Societies are time-tested legal structures for running educational institutions, hospitals, temples, cultural bodies, and community welfare programs.
              </p>
              <div className="grid grid-cols-1 gap-4">
                {[
                  { t: "12A & 80G Tax Benefits", d: "Exemption from income tax on donations and receipts, plus 50% tax deductions for donors." },
                  { t: "Institutional Asset Holding", d: "Hold immovable property, schools, land, and endowment funds in perpetuity." },
                  { t: "Democratic Governance", d: "Societies provide democratic election of President, Secretary, and Treasurer." },
                  { t: "Government Grants & DARPAN", d: "Eligible for central and state government social welfare funds and CSR grants." }
                ].map((item, i) => (
            ...
            <div className="md:w-1/2">
              <div className="bg-slate-50 p-8 rounded-3xl border border-slate-200 shadow-sm">
                <h3 className="text-xl font-black text-slate-900 mb-6 text-center">Trust & Society Requirements</h3>
                <ul className="space-y-4">
                  {[
                    "Trust: Minimum 2 Trustees | Society: Minimum 7 Members",
                    "Customized Trust Deed / Society Memorandum & By-laws",
                    "PAN & Aadhaar Cards of all Trustees / Members",
                    "Registered Address Electricity Bill & Landlord NOC",
                    "State Non-Judicial Stamp Paper & 2 Witness KYC Proofs"
                  ].map((req, i) => (
              ...
              <div className="mt-12 text-left bg-slate-50 p-8 md:p-12 rounded-3xl border border-slate-200 animate-in fade-in slide-in-from-top-4 duration-300 grid grid-cols-1 lg:grid-cols-3 gap-12">
                ...
                <div className="lg:col-span-1 space-y-6 max-h-[500px] overflow-y-auto pr-4 scrollbar-thin scrollbar-thumb-slate-300">
                  <h3 className="text-xl font-black text-slate-900 border-b-2 border-red-500 pb-2">
                    {activeGuide.title || 'Guide to Society & Trust Registration'}
                  </h3>
        ...
        <section className="py-24 bg-slate-900 text-white relative overflow-hidden">
          ...
          <div className="max-w-4xl mx-auto px-4 text-center relative z-10">
            ...
            <p className="text-xl text-slate-400 mb-10 leading-relaxed font-medium">
              Talk to our trust and society legal advisors before you execute deeds. Pay a small booking fee now, and we will deduct it from your final bill.
            </p>
...
export default SocietyTrustRegistrationPage;
