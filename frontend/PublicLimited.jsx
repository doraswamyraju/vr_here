...
import { publicLimitedConfig } from '../backend/data/serviceConfigs/publicLimited.js';

const PAGE_ID = 'public-limited-company';

const RELATED_SERVICES = [
  {
    title: "Private Limited Company",
    price: "₹6,499",
    desc: "The gold standard for venture-backed startups and growth enterprises with up to 200 shareholders.",
    link: "/pvt-ltd-registration"
  },
  {
    title: "Limited Liability Partnership (LLP)",
    price: "₹7,899",
    desc: "Ideal for professional partnerships needing limited liability with zero mandatory audit under ₹40 Lakhs.",
    link: "/llp-registration"
  },
  {
    title: "Section 8 Company (NGO)",
    price: "₹12,499",
    desc: "Non-profit corporate structure with nationwide reach, CSR funding eligibility, and 12A/80G tax exemptions.",
    link: "/section-8-company"
  },
  {
    title: "GST Registration & Filing",
    price: "₹2,569",
    desc: "Complete GSTIN registration and initial return filing support handled seamlessly by expert CAs.",
...
const PublicLimitedPage = () => {
  ...
  const [pageConfig, setPageConfig] = useState(publicLimitedConfig);
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      const mainEl = document.getElementById('public-limited-container');
      if (mainEl) {
  ...
  const activeHero = pageConfig?.hero || publicLimitedConfig.hero;
  const activeStats = pageConfig?.stats || publicLimitedConfig.stats;
  const activeLogos = pageConfig?.logos || publicLimitedConfig.logos;
  const activePackages = pageConfig?.packages || publicLimitedConfig.packages;
  const activeReviews = pageConfig?.reviews || publicLimitedConfig.reviews;
  const activeSteps = pageConfig?.steps || publicLimitedConfig.steps;
  const activeFaqs = pageConfig?.faqs || publicLimitedConfig.faqs;
  const activeGuide = pageConfig?.guide || publicLimitedConfig.guide;
  const activePopularSearches = pageConfig?.popularSearches || publicLimitedConfig.popularSearches;
    ...
    launchRazorpayCheckout({
      ...
      onSuccess: async (data) => {
        ...
        await showPaymentSuccessPopup({
          serviceName: selectedPlan?.name || data?.order?.serviceName || 'Public Limited Company Registration',
  ...
  return (
    <div className="font-sans text-slate-800 bg-white min-h-screen selection:bg-red-100 selection:text-red-900 overflow-x-hidden">
      ...
      <ConsultationPaymentModal
        ...
        title={selectedPlan?.id === 'consultation' ? 'Book Public Co Consultation' : 'Select Formation Package'}
        initialTermsAccepted={false}
      />

      <div id="public-limited-container" className="animate-fade-in">
        {/* 1. Hero Section */}
          ...
          <div className="max-w-7xl mx-auto px-4 relative z-10 flex flex-col lg:flex-row items-center gap-12">
            <div className="lg:w-1/2">
              <div className="inline-flex items-center px-4 py-1.5 rounded-full bg-white border border-slate-200 shadow-sm text-sm font-bold text-slate-600 mb-6">
                ...
                {sanitizeText(activeHero.badgeText, pageConfig?.city) || 'Enterprise-Grade Incorporation'}
              </div>
              <h1 className="text-4xl lg:text-6xl font-black text-slate-900 tracking-tight leading-[1.1] mb-6">
                {sanitizeText(activeHero.title, pageConfig?.city) || 'Register Public Limited Company Online'}
              </h1>
              <p className="text-xl text-slate-600 leading-relaxed mb-8">
                {sanitizeText(activeHero.subtitle, pageConfig?.city) || 'Launch an enterprise-grade public corporation with unlimited shareholders, debentures & capital access.'}
              </p>
            ...
            <div className="lg:w-1/2 relative hidden lg:block">
              <div className="relative z-10 bg-white p-8 rounded-2xl shadow-2xl border border-slate-100 transform rotate-2 hover:rotate-0 transition duration-500">
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <h3 className="text-xl font-bold text-slate-900">Public Limited Setup</h3>
                    <p className="text-slate-500 text-sm">Enterprise statutory formation from ₹14,999</p>
                  </div>
                  <div className="bg-green-100 text-green-700 font-bold px-3 py-1 rounded text-xs">MCA VERIFIED</div>
                </div>
                <div className="space-y-4">
                  {['7+ Shareholders / Subscribers Setup', '3 DINs & 3 Class 3 DSCs Included', 'MCA RUN Public Name Approval', 'Custom Public Co MOA & AOA Drafting', 'PAN, TAN & INC-20A Commencement Kit'].map((item, i) => (
        ...
        <div className="bg-slate-900 py-12 relative overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto px-4 mb-6 text-center">
            <p className="text-xs uppercase font-black tracking-widest text-red-400">
              Trusted By Over 5,000+ Fast-Growing Indian Startups & Enterprises
            </p>
          </div>
                ...
                >
                  ...
                  <div className="text-4xl font-black text-slate-900 mb-6">
                    ...
                    <span className="text-[10px] font-bold text-slate-500 ml-1 block mt-1">
                      {pkg.isAdjustable ? '(Fully Adjustable)' : '+ Govt Fees & Stamp Duty'}
                    </span>
                  </div>
        ...
        <section className="py-20 bg-white border-y border-slate-100">
          <div className="max-w-7xl mx-auto px-4">
            <div className="text-center mb-16">
              ...
              <p className="text-lg text-slate-600 mt-2 font-medium">Hear directly from corporations registered and powered by VR Here.</p>
            </div>
        ...
        <section className="py-24 bg-slate-50">
          <div className="max-w-6xl mx-auto px-4">
            <div className="text-center mb-20">
              ...
              <p className="text-lg text-slate-600 mt-2 font-medium">Enterprise corporate formation managed 100% digitally by legal specialists.</p>
            </div>
        ...
        {/* 6. Why Public Limited & Requirements */}
          ...
          <div className="max-w-7xl mx-auto px-4 flex flex-col md:flex-row gap-12 items-center">
            <div className="md:w-1/2">
              ...
              <p className="text-slate-600 mb-8 leading-relaxed font-medium">
                A Public Limited Company is the vehicle of choice for large-scale operations, public capital raising, and enterprise contracts.
              </p>
              <div className="grid grid-cols-1 gap-4">
                {[
                  { t: "Public Equity & Debentures", d: "Legally raise funds from institutions, investors, and public deposits." },
                  { t: "Free Share Transferability", d: "Shares can be freely transferred without restrictions of private entities." },
                  { t: "Maximum Market Credibility", d: "Unmatched stature with government departments, global banks, and tenders." },
                  { t: "Perpetual Corporate Existence", d: "Uninterrupted life unaffected by changes in directors or shareholders." }
                ].map((item, i) => (
            ...
            <div className="md:w-1/2">
              <div className="bg-slate-50 p-8 rounded-3xl border border-slate-200 shadow-sm">
                <h3 className="text-xl font-black text-slate-900 mb-6 text-center">Public Co Minimum Requirements</h3>
              ...
              <div className="mt-12 text-left bg-slate-50 p-8 md:p-12 rounded-3xl border border-slate-200 animate-in fade-in slide-in-from-top-4 duration-300 grid grid-cols-1 lg:grid-cols-3 gap-12">
                ...
                <div className="lg:col-span-1 space-y-6 max-h-[500px] overflow-y-auto pr-4 scrollbar-thin scrollbar-thumb-slate-300">
                  <h3 className="text-xl font-black text-slate-900 border-b-2 border-red-500 pb-2">
                    {activeGuide.title || 'Guide to Public Limited Company Registration'}
                  </h3>
        ...
        <section className="py-24 bg-slate-900 text-white relative overflow-hidden">
          ...
          <div className="max-w-4xl mx-auto px-4 text-center relative z-10">
            ...
            <p className="text-xl text-slate-400 mb-10 leading-relaxed font-medium">
              Talk to our corporate secretaries before you commit. Pay a small booking fee now, and we will deduct it from your final bill.
            </p>
