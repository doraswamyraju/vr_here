...
const FreelancerApp = () => {
  ...
  return (
    <div className="flex h-screen bg-gradient-to-br from-slate-100 via-blue-50 to-indigo-100 font-sans text-slate-800 overflow-hidden">
      <aside
        className={`${sidebarCollapsed ? 'w-20' : 'w-72'} bg-white/75 backdrop-blur-md h-full border-r border-slate-200/70 flex flex-col z-20 transition-all duration-300`}
      ...
      >
        ...
        <div className="flex-1 py-5 px-3 space-y-1 overflow-y-auto">
          {freelancerSidebarTabs.map((item) => {
            ...
            return (
              ...
              >
                ...
                <span className={`ml-3 whitespace-nowrap transition-all duration-300 ${sidebarCollapsed ? 'opacity-0 w-0 overflow-hidden' : 'opacity-100'}`}>
                  {item.label}
                </span>
              </button>
            );
        ...
        <div className="p-4 border-t border-slate-100">
          <button onClick={handleLogout} className="flex items-center w-full p-2 rounded-lg text-rose-500 hover:bg-rose-50">
            <LogOut size={20} />
            <span className={`ml-3 text-sm transition-all duration-300 ${sidebarCollapsed ? 'opacity-0 w-0 overflow-hidden' : 'opacity-100'}`}>
              Logout
            </span>
          </button>
        </div>
