...
import { proprietorshipConfig } from '../backend/data/serviceConfigs/proprietorship.js';

const PAGE_ID = 'proprietorship-setup';

const RELATED_SERVICES = [
  {
    title: "Partnership Firm",
    price: "₹3,499",
    desc: "Drafting, stamp notary, and PAN creation for 2 or more business partners.",
    link: "/partnership-firm-registration"
  },
  {
    title: "One Person Company (OPC)",
    price: "₹5,999",
    desc: "Upgrade to limited liability protection and corporate status while staying solo.",
    link: "/one-person-company"
  },
  {
    title: "GST Registration & Filing",
    price: "₹2,569",
    desc: "Mandatory for selling interstate or via e-commerce (Amazon, Flipkart).",
    link: "/gst-registration"
  },
  {
    title: "Private Limited Company",
    price: "₹6,499",
    desc: "The premier startup structure with full equity investor capabilities.",
...
const ProprietorshipSetupPage = () => {
  const navigate = useNavigate();

  const userInfo = JSON.parse(localStorage.getItem('userInfo') || 'null');
  const isAuthorized = userInfo && (userInfo.role === 'admin' || userInfo.role === 'employee');

  const [pageConfig, setPageConfig] = useState(proprietorshipConfig);
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      const mainEl = document.getElementById('proprietorship-container');
      if (mainEl) {
  ...
  const activeHero = pageConfig?.hero || proprietorshipConfig.hero;
  const activeStats = pageConfig?.stats || proprietorshipConfig.stats;
  const activeLogos = pageConfig?.logos || proprietorshipConfig.logos;
  const activePackages = pageConfig?.packages || proprietorshipConfig.packages;
  const activeReviews = pageConfig?.reviews || proprietorshipConfig.reviews;
  const activeSteps = pageConfig?.steps || proprietorshipConfig.steps;
  const activeFaqs = pageConfig?.faqs || proprietorshipConfig.faqs;
  const activeGuide = pageConfig?.guide || proprietorshipConfig.guide;
  const activePopularSearches = pageConfig?.popularSearches || proprietorshipConfig.popularSearches;
    ...
    launchRazorpayCheckout({
      ...
      onSuccess: async (data) => {
        ...
        await showPaymentSuccessPopup({
          serviceName: selectedPlan?.name || data?.order?.serviceName || 'Sole Proprietorship Registration',
  ...
  return (
    <div className="font-sans text-slate-800 bg-white min-h-screen selection:bg-red-100 selection:text-red-900 overflow-x-hidden">
      ...
      <ConsultationPaymentModal
        ...
        title={selectedPlan?.id === 'consultation' ? 'Book Proprietor Advisory' : 'Select Proprietorship Package'}
        initialTermsAccepted={false}
      />

      <div id="proprietorship-container" className="animate-fade-in">
        {/* 1. Hero Section */}
          ...
          <div className="max-w-7xl mx-auto px-4 relative z-10 flex flex-col lg:flex-row items-center gap-12">
            <div className="lg:w-1/2">
              <div className="inline-flex items-center px-4 py-1.5 rounded-full bg-white border border-slate-200 shadow-sm text-sm font-bold text-slate-600 mb-6">
                ...
                {sanitizeText(activeHero.badgeText, pageConfig?.city) || 'Ready in 48 Hours'}
              </div>
              <h1 className="text-4xl lg:text-6xl font-black text-slate-900 tracking-tight leading-[1.1] mb-6">
                {sanitizeText(activeHero.title, pageConfig?.city) || 'Sole Proprietorship Registration Online'}
              </h1>
              <p className="text-xl text-slate-600 leading-relaxed mb-8">
                {sanitizeText(activeHero.subtitle, pageConfig?.city) || 'Fastest way to start your business as a single owner with zero compliance burden.'}
              </p>
              ...
              <div className="flex flex-col sm:flex-row gap-4 mb-8">
                ...
                >
                  Book Quick Call @ ₹{activeHero.consultationPrice || 299} <ArrowRight className="ml-2 w-5 h-5" />
                </button>
              ...
              <div className="flex items-center space-x-2 text-sm font-medium text-slate-500">
                ...
                <span>Consultation fee of ₹{activeHero.consultationPrice || 299} is 100% adjustable in final package</span>
              </div>
            </div>
            ...
            <div className="lg:w-1/2 relative hidden lg:block">
              <div className="relative z-10 bg-white p-8 rounded-2xl shadow-2xl border border-slate-100 transform rotate-2 hover:rotate-0 transition duration-500">
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <h3 className="text-xl font-bold text-slate-900">Proprietorship Kit</h3>
                    <p className="text-slate-500 text-sm">All-inclusive starting at ₹1,499</p>
                  </div>
                  <div className="bg-green-100 text-green-700 font-bold px-3 py-1 rounded text-xs">48 HR DISPATCH</div>
                </div>
                <div className="space-y-4">
                  {['Udyam MSME Government Registration', 'GSTIN Application (TRN & ARN)', 'Shop & Establishment Act / Trade Proof', 'Current Bank Account Resolution Kit', 'Professional Invoice Billing Template'].map((item, i) => (
        ...
        <div className="bg-slate-900 py-12 relative overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto px-4 mb-6 text-center">
            <p className="text-xs uppercase font-black tracking-widest text-red-400">
              Trusted By Over 5,000+ Fast-Growing Indian Startups & Traders
            </p>
          </div>
                ...
                >
                  ...
                  <div className="text-4xl font-black text-slate-900 mb-6">
                    ...
                    <span className="text-[10px] font-bold text-slate-500 ml-1 block mt-1">
                      {pkg.isAdjustable ? '(Fully Adjustable)' : 'All-Inclusive Government Licensing'}
                    </span>
                  </div>
        ...
        <section className="py-20 bg-white border-y border-slate-100">
          <div className="max-w-7xl mx-auto px-4">
            <div className="text-center mb-16">
              ...
              <h2 className="text-3xl lg:text-5xl font-black text-slate-900 mt-4 tracking-tight">What Our Clients Say</h2>
              <p className="text-lg text-slate-600 mt-2 font-medium">Hear directly from traders and solo entrepreneurs launched by VR Here.</p>
            </div>
        ...
        <section className="py-24 bg-slate-50">
          <div className="max-w-6xl mx-auto px-4">
            <div className="text-center mb-20">
              <span className="text-xs uppercase font-black tracking-widest text-red-600 bg-red-50 px-3 py-1.5 rounded-full font-bold">Launch Flow</span>
              <h2 className="text-3xl lg:text-5xl font-black text-slate-900 mt-4 tracking-tight">How It Works in 48 Hours</h2>
              <p className="text-lg text-slate-600 mt-2 font-medium">Get licensed and start banking from the comfort of your home.</p>
            </div>
        ...
        {/* 6. Why Sole Proprietorship & Requirements */}
          ...
          <div className="max-w-7xl mx-auto px-4 flex flex-col md:flex-row gap-12 items-center">
            <div className="md:w-1/2">
              ...
              <p className="text-slate-600 mb-8 leading-relaxed font-medium">
                The fastest, most cost-effective path to launch your commercial venture with full autonomous ownership.
              </p>
              <div className="grid grid-cols-1 gap-4">
                {[
                  { t: "100% Single Ownership", d: "You retain complete control over business decisions and profits." },
                  { t: "Minimal Compliance Burden", d: "No annual board meetings, secretarial audits, or MCA balance sheet filings." },
                  { t: "Direct Individual Tax Benefits", d: "Profits are taxed at normal individual personal income tax slab rates." },
                  { t: "Easy Conversion Later", d: "Seamlessly convert into an OPC or Private Limited company as revenues scale." }
                ].map((item, i) => (
            ...
            <div className="md:w-1/2">
              <div className="bg-slate-50 p-8 rounded-3xl border border-slate-200 shadow-sm">
                <h3 className="text-xl font-black text-slate-900 mb-6 text-center">RBI Bank KYC Requirements</h3>
                <ul className="space-y-4">
                  {[
                    "Single Individual Owner (PAN & Aadhaar)",
                    "Udyam / MSME Registration Certificate",
                    "GSTIN Certificate or Shop & Establishment License",
                    "Commercial / Residential Address Electricity Bill",
                    "Savings Account Statement / Cancelled Cheque"
                  ].map((req, i) => (
              ...
              <div className="mt-12 text-left bg-slate-50 p-8 md:p-12 rounded-3xl border border-slate-200 animate-in fade-in slide-in-from-top-4 duration-300 grid grid-cols-1 lg:grid-cols-3 gap-12">
                ...
                <div className="lg:col-span-1 space-y-6 max-h-[500px] overflow-y-auto pr-4 scrollbar-thin scrollbar-thumb-slate-300">
                  <h3 className="text-xl font-black text-slate-900 border-b-2 border-red-500 pb-2">
                    {activeGuide.title || 'Guide to Sole Proprietorship Setup'}
                  </h3>
        ...
        <section className="py-24 bg-slate-900 text-white relative overflow-hidden">
          ...
          <div className="max-w-4xl mx-auto px-4 text-center relative z-10">
            ...
            <p className="text-xl text-slate-400 mb-10 leading-relaxed font-medium">
              Talk to our CA specialists before applying. Pay a small booking fee now, and we will deduct it from your final bill.
            </p>
            <div className="bg-white/10 backdrop-blur-md p-8 rounded-3xl border border-white/10 inline-block w-full max-w-md">
              ...
              <div className="text-5xl font-black mb-2">₹{activeHero.consultationPrice || 299}</div>
...
export default ProprietorshipSetupPage;
